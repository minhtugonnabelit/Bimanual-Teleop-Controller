...
class PR2BimanualController:
    ...
    def single_joint_test(self):
        ...
        while not rospy.is_shutdown():
            ...
            dir = self._joy[0][1] / np.abs(self._joy[0]
                                           [1]) if np.abs(self._joy[0][1]) > 0.1 else 0
            if self._joy[1][4]:
            ...
            if not (self._joy[1][4] + self._joy[1][5]):
                qdot = np.zeros(14)

            self._arm_vel_pub['left'].publish(
                PR2BimanualController.joint_group_command_to_msg(qdot[7:]))
            self._arm_vel_pub['right'].publish(
                PR2BimanualController.joint_group_command_to_msg(qdot[:7]))
