...
class PR2Controller:
    ...
    def __init__(self, rate, joystick : JoystickController, config, data_plotter=False):
        ...
        self._head_controller_pub = rospy.Publisher('/head_traj_controller/command', JointTrajectory, queue_size=10)
    ...
    def get_joint_angles(self, joint_names: list):
        return [self._joint_states.position[self._joint_states.name.index(joint)] for joint in joint_names]

    def move_head(self, vel_cmd):
    ...
    def store_joint_velocities(self, side: str, qdot: list):
        ...
        # qdot_store = CalcFuncs.reorder_values(
        #     self._joint_states.velocity[31:38]) if side=="left" else CalcFuncs.reorder_values(
        #     self._joint_states.velocity[17:24])
        # self._qdot_record[side].append(qdot_store) 
